...
func (n *NodeApi) StoreChainContract(c *gin.Context) {
	var res common.Response
	...
	rd, _ := json.Marshal(contract)
	global.NODE_LOG.Info("StoreChainContract: " + string(rd))

	err = service.NodeService.StoreChainContract(contract)
	if err != nil {
		global.NODE_LOG.Error(err.Error(), zap.Error(err))
		res = common.FailWithMessage(err.Error())
		c.JSON(http.StatusOK, res)
...
func (n *NodeApi) BulkStoreChainContract(c *gin.Context) {
	var res common.Response
	...
	rd, _ := json.Marshal(contracts)
	global.NODE_LOG.Info("BulkStoreChainContract: " + string(rd))

	result, err := service.NodeService.BulkStorageChainContract(contracts)
	if err != nil {
		global.NODE_LOG.Error(err.Error(), zap.Error(err))
		res = common.FailWithDetailed(common.Error, err.Error(), result)
		c.JSON(http.StatusOK, res)
...
// func (n *NodeApi) GetTransactionByChainAndHash(c *gin.Context) {
// 	var res common.Response
...
// 	global.NODE_LOG.Info("GetTransactionByChainAndHash: " + string(rd))

// 	result, err := service.NodeService.GetTransactionByChainAndHash(tx.ChainId, tx.Hash)
// 	if err != nil {
// 		global.NODE_LOG.Error(err.Error(), zap.Error(err))
// 		res = common.FailWithDetailed(common.Error, err.Error(), result)
// 		c.JSON(http.StatusOK, res)
